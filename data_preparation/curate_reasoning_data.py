...
def main(args: argparse.Namespace) -> None:  # noqa: C901, PLR0915
    ...
    dataset = DocumentDataset.read_json(
        input_files, files_per_partition=args.json_files_per_partition
    )
    ...
    output_path = args.output_dir + "/output_*.jsonl"
    interleaved_df = interleaved_df.repartition(npartitions=1)
    interleaved_df.to_json(output_path, orient="records", lines=True)
    print(f"Saved sorted dataset to {output_path}")

    # Start splitting into train/val/test
    print("Splitting dataset into train/validation/test")

    # Load back the output file (optional, or you can directly use interleaved_df)
    df = interleaved_df.compute()  # convert to pandas dataframe
    ...
    # Save splits
    train_df.to_json(os.path.join(args.output_dir, "training.jsonl"), orient="records", lines=True)
    val_df.to_json(os.path.join(args.output_dir, "validation.jsonl"), orient="records", lines=True)
    test_df.to_json(os.path.join(args.output_dir, "test.jsonl"), orient="records", lines=True)
